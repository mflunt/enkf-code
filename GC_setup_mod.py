...
def create_prior_covariance(x_ap, sigma_x, lat_state, lon_state, correlated=False, corr_type="exp", length_scale = 250):
    ...
    if correlated == False:
        ...
        for ti in range(nstate):
            ...
            deltaspace[ti,:] = haversine.multiple_dist_arrays([lat_state[ti],lon_state[ti]], 
                      lat_state,lon_state)
...
def write_input_file(start_date, end_date, spc_names, bc_dir, spc_BC,
                     ...
                     global_res, species, region, post_run=False):
    """
    ...
    template_file = template_dir + "input_ensemble_" + species + ".template"
    if global_res == "2x2.5":
        T_F_res = "T"
    else:
        T_F_res = "F"
   ...
   # end_date = pd_end.strftime('%Y%m%d')
    ...
    with open(run_dir + "input.geos", "w") as out_file:
        for line in filedata:
            ...
            if "MetRoot" in line:
                line = line.replace('{MetRoot}', met_root)
            if "Ioffset" in line:
                line = line.replace('{Ioffset}', Ioffset)
                line = line.replace('{Joffset}', Joffset)
            if "NumberAdvectedSpecies" in line:
                line = line.replace('{NumberAdvectedSpecies}', str(nAdvect))
                
            if "Tstep_transport" in line:
                line = line.replace('{Tstep_transport}', tstep_transport) 
            if "Tstep_convect" in line:
                line = line.replace('{Tstep_convect}', tstep_convect) 
            if "Tstep_emis" in line:
                line = line.replace('{Tstep_emis}', tstep_emis) 
            if "Tstep_chem" in line:
                line = line.replace('{Tstep_chem}', tstep_chem) 
            ...
            if "Over North America" in line:
                if region == "North America":
                    line = line.replace("F", "T" )
            if "Over Europe" in line:
                if region == "EUROPE":
                    line = line.replace("F", "T" )
            if "Over China" in line:
                if region == "CHINA":
                    line = line.replace("F", "T" )
            if "Over Asia" in line:
                if region in (["ASIA", "AFRICA"]):
                    line = line.replace("F", "T" )
...
def write_hemco_config(start_date, end_date, spc_emit, fname_scale_factors,
                       run_dir, template_dir, species, post_run = False):
    """
    ...
    template_file = template_dir + "HEMCO_Config_ensemble_" + species + ".template"
    #run_dir = "/geos/d21/mlunt/gc11_rundirs/" + run_name + "/"
    #template_dir = "/home/mlunt/programs/GC/rundirs/templates/EnKF/"
...
#    else:
#        days_in_end_month = days_in_month(pd_end.month)
#        days_in_lag_month = days_in_month(pd_lag_start.month)
#        string_list=[]
#        string_list2=[]
#        for ti in range(days_in_end_month):
#            string_list.append('0')
#        for ti in range(days_in_lag_month):
#            string_list2.append('0')
#        
#        string_list[pd_end.day-1] = '3' 
#        string_list2[pd_lag_start.day-1] = '3'
#        string_30s = "".join(string_list)
#        string_30s2 = "".join(string_list2)
        
    
#    if pd_end.month in ([1,3,5,7,8,10,12]):
#        days_in_end_month = 31
#    elif pd_end.month in ([4,6,9,11]):
#        days_in_end_month = 30
#    else:
#        days_in_end_month = 29
    
    # What if lag start and end_date are in the same month?
#    string_list=[]
#    string_list2=[]
#    for ti in range(days_in_end_month):
#        string_list.append('0')
